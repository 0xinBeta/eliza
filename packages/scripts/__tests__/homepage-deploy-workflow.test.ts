...
describe("homepage deployment workflow", () => {
  ...
  it("retires every standalone homepage application lifecycle", () => {
    ...
    expect(completeWorkflow).not.toContain("eliza-app-home");
  ...
  it("builds homepage changes into the single eliza-app artifact", () => {
    ...
    expect(completeWorkflow).toContain("https://eliza.app");
    expect(completeWorkflow).toContain("https://cloud.eliza.app");
    expect(completeWorkflow).toContain("https://staging.eliza.app");
    expect(completeWorkflow).toContain("https://cloud-staging.eliza.app");
  });
  ...
  it("resolves matched Telegram configuration for canonical and preview builds", () => {
    ...
    expect(completeWorkflow.match(/TELEGRAM_BOT_ID=7684336618/g)).toHaveLength(
      2,
    );
    expect(
      completeWorkflow.match(/TELEGRAM_BOT_USERNAME=Elizav2_Bot/g),
    ).toHaveLength(2);
    expect(
      completeWorkflow.match(
